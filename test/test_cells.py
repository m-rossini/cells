...
import spike.life.parameters

def test_cells_pool():
    c = Cell([])
    assert 12 == len(c.genes)

    for gene in c.genes.values():
        if gene.gene_type not in [GeneType.ABILITY_TO_REPRODUCE, GeneType.GENDER_FLUIDITY]:
            assert gene.gene_value >= spike.life.parameters.MIN_RANDOM_GENE_VALUE and gene.gene_value <= spike.life.parameters.MAX_RANDOM_GENE_VALUE
